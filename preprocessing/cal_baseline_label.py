...
from preprocessing.utils import freq_moving_ave
...
def cal_pcr_label(df_dat):
    df_dat['PCR'] = np.nan_to_num(df_dat['play_time_truncate'].values / df_dat['duration_ms'].values, nan=0.0,
...
def cal_gain_label(df_dat, dat_name):
    ...
    df_dat['gain'] = np.nan_to_num(
        (df_dat['play_time_truncate'].values - df_dat['mean_play'].values) / df_dat['std_play'].values, nan=0.0,
        posinf=0.0, neginf=0.0)
...
def cal_d2co_label(df_dat, windows_size, alpha):
    ...
    for d in tqdm(np.arange(min_duration, max_duration + 1, 1)):
        ...
        # all duration>5 
        if len(X) > 2:
            gm = GaussianMixture(n_components=2, init_params='kmeans', covariance_type='spherical', max_iter=500,
                                 random_state=61).fit(X)
    ...
    df_stat = df_dat[(df_dat['duration_ms'] <= max_duration) & (df_dat['duration_ms'] >= min_duration)][
        'duration_ms'].value_counts()
    ...
    df_dat['D2Co'] = (df_dat['play_time_truncate'] - df_dat['nega_mean']) / (
                df_dat['posi_mean'] - df_dat['nega_mean'] + 1e-6)
